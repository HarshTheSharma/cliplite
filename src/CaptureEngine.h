...
struct VideoFrame {
    ...
    ID3D11Texture2D*      texture  = nullptr; // raw borrow; valid while pool_ref lives
    std::shared_ptr<void> pool_ref;            // dtor returns texture to TexturePool
...
// Video  : 30 fps × 320 s headroom → 9600 → round up to 16384
