...
AudioMic::AudioMic(CaptureEngine& engine) : engine_(engine) {}
AudioMic::~AudioMic() { Stop(); }

bool AudioMic::Initialize() {
...
void AudioMic::Start() {
    ...
    client_->Start();
    running_ = true;
    thread_ = std::thread(&AudioMic::CaptureLoop, this);
}
...
void AudioMic::CaptureLoop() {
    ...
    while (running_) {
        ...
        while (SUCCEEDED(capture_->GetNextPacketSize(&frames_avail)) && frames_avail) {
            ...
            engine_.GetMicRing().push(std::move(pkt));
        }
    }
